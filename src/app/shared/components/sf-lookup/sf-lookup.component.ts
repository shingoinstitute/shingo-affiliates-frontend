...
export class SfLookupComponent
  ...
  public queryHandler: Observable<
    string
  > = this.queryHandlerSource.asObservable()
  ...
  public ngAfterViewInit() {
    ...
    this.lookup.controls.sfObject.valueChanges
      .pipe(filter(query => query && query.length > 2))
    ...
    if (query && query.length > 2) {
      ...
      if (this.sfObject instanceof Affiliate) {
        if (!this.fields || !this.fields.length)
          this.fields = DEFAULT_AFFILIATE_SEARCH_FIELDS
      ...
      if (!searchFn) {
        throw new Error(
          `Cannot determine a search function for ${typeof this.sfObject}`,
        )
      }
