...
const fileAcceptable = (accept: string[]) => (file: File): boolean => {
  ...
  return accept.includes(file.type) || accept.includes(extension) || testGlob(accept)(file.type);
};

const validateFile = (accept: string[], maxSize: number) => (file: File): Either<FileFailure, File> => {
...
const supportsDnD = () => {
  ...
  const supported = ('draggable' in div) || ('ondragstart' in div && 'ondrop' in div);
  return supported;
};
...
export class FileDropComponent {
  ...
  @Input() public multiple: boolean | number = false;
  @Input() public maxTotalSize = Infinity;
  @Input() public maxFileSize = Infinity;
  ...
  private handleFiles(files: File[]) {
    ...
    const multiple = typeof this.multiple === 'boolean'
      ? this.multiple ? Infinity : 1
      : this.multiple;

    const fn = compose(
      mapEither((file: File) => this.handleFile(file), (err: FileFailure) => this.handleError(err)),
      chainEither((file: File): Either<FileFailure, File> => {
        count++;
        size += file.size;
