...
export class WorkshopDataTableComponent implements OnInit {
  ...
  public formatDaysLate(w: Workshop): string {
    const daysLate = this.getDaysLate(w)
    if (daysLate < 1) return `in ${Math.abs(daysLate)} days`
    if (daysLate === 1) return `1 day ago`
    if (daysLate < 30) return `${daysLate} days ago`
    if (daysLate < 60) return `1 month ago`
    if (daysLate < 365) return `${Math.floor(daysLate / 30)} months ago`
    if (daysLate < 365 + 30) return `1 year ago`
    if (daysLate < 365 * 2) {
