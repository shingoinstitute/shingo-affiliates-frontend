...
when(mockedService.getById('some sf id')).thenReturn(observableOf(EXPECTED_AFFILIATES[0]));
when(mockedService.create(anyOfClass(Affiliate))).thenReturn(observableOf(EXPECTED_SF_SUCCESS));
when(mockedService.update(anyOfClass(Affiliate))).thenReturn(observableOf(EXPECTED_SF_SUCCESS));
when(mockedService.delete(anyOfClass(Affiliate))).thenReturn(observableOf(EXPECTED_SF_SUCCESS));
when(mockedService.search(anyString())).thenReturn(observableOf(EXPECTED_AFFILIATES));
when(mockedService.describe()).thenReturn(observableOf(EXPECTED_DESCRIBE));
when(mockedService.searchCMS(anyString(), anyString())).thenReturn(observableOf(EXPECTED_COURSE_MANAGERS));
when(mockedService.map(anyOfClass(Affiliate))).thenReturn(observableOf({ mapped: true as true }));
