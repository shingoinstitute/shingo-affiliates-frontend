...
when(mockedService.getById('some sf id')).thenReturn(observableOf(EXPECTED_WORKSHOPS[0]));
when(mockedService.create(anyOfClass(Workshop))).thenReturn(observableOf(EXPECTED_SF_SUCCESS));
when(mockedService.update(anyOfClass(Workshop))).thenReturn(observableOf(EXPECTED_SF_SUCCESS));
when(mockedService.delete(anyOfClass(Workshop))).thenReturn(observableOf(EXPECTED_SF_SUCCESS));
when(mockedService.search(anyString())).thenReturn(observableOf(EXPECTED_WORKSHOPS));
