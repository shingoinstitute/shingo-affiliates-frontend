...
export const EXPECTED_WORKSHOPS: Workshop[] = [new Workshop({ Id: 'some sf id' }), new Workshop({ Id: 'some other sf id' })];
...
when(mockedService.getById('some sf id')).thenReturn(Observable.of(EXPECTED_WORKSHOPS[0]));
when(mockedService.create(anyOfClass(Workshop))).thenReturn(Observable.of(EXPECTED_SF_SUCCESS));
when(mockedService.update(anyOfClass(Workshop))).thenReturn(Observable.of(EXPECTED_SF_SUCCESS));
when(mockedService.delete(anyOfClass(Workshop))).thenReturn(Observable.of(EXPECTED_SF_SUCCESS));
when(mockedService.search(anyString())).thenReturn(Observable.of(EXPECTED_WORKSHOPS));
