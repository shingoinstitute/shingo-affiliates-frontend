...
when(mockedService.getById('some sf id')).thenReturn(observableOf(EXPECTED_FACILITATORS[0]));
when(mockedService.create(anyOfClass(Facilitator))).thenReturn(observableOf(EXPECTED_SF_SUCCESS));
when(mockedService.update(anyOfClass(Facilitator))).thenReturn(observableOf(EXPECTED_SF_SUCCESS));
when(mockedService.delete(anyOfClass(Facilitator))).thenReturn(observableOf(EXPECTED_SF_SUCCESS));
when(mockedService.search(anyString(), anything(), anything())).thenReturn(observableOf(EXPECTED_FACILITATORS));
