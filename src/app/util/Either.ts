...
const mapEither = <L, R, RR = R, LL = L>(mright: ((v: R) => RR),
                                   mleft?: ((v: L) => LL)) =>
  ...
  if (isLeft(either)) {
    return typeof mleft === 'function' ? left(mleft(either[1])) : either as any as Left<LL>;
  }
};

const chainEither = <L, R, RR = R>(fr: ((v: R) => Either<L, RR>)) => (either: Either<L, R>): Either<L, RR> => {
  if (isRight(either)) {
    return fr(either[1]);
  }
